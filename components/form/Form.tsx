...
export function Form({
	...
	const onSubmitHandler = async (e: FormEvent<HTMLFormElement>) => {
		...
		if (
			honeyPotFieldName &&
			new FormData(e.target as HTMLFormElement).get(honeyPotFieldName)
		) {
		...
		>
			{honeyPotFieldName && (
				<input
					...
					className="form__control--special"
				/>
			)}
			{children}
		</motion.form>
...
export namespace Form {
	...
	const getErrorMessage = (
		...
		if (/\./.test(targetFieldName)) {
			return getErrorMessage(
				targetFieldName.split(".")[1],
				value[targetFieldName.split(".")[0]]
			);
		}
	...
	export const Submit = ({
		formState,
		...props
	}: ButtonProps & { formState: FormState<any> }) => (
		<Button
			{...props}
			type="submit"
			disabled={
